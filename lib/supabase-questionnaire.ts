...
export async function submitQuestionnaire(state: QuestionnaireState) {
  ...
  const { error } = await getSupabaseClient().from("questionnaire_responses").insert({
    ...
    qualities: a.qualities.map((value) => value === "Autre" && a.qualitiesAutre ? `Autre : ${a.qualitiesAutre}` : value),
    defauts: a.defauts.map((value) => value === "Autre" && a.defautsAutre ? `Autre : ${a.defautsAutre}` : value),
